...
async function loadSessionAndEmployee() {
  ...
  try {
    ...
    if (empErr || !emp) {
      ...
      const r = await supabase.from("employees").select("employee_uid, full_name").eq("email", st.user.email).single();
      emp = r.data || null;
      empErr = r.error || null;
    }
...
function renderSummary(record, uiTypeHint = null) {
  ...
  // VACACIONES: mostrar solo lo que exista en el registro (sin calcular)
  if (dbType === "VACACIONES") {
    ...
    // Campos opcionales si existen en tu tabla/vista
    if ("holidays_in_range" in record) inner += fields("Feriados en rango", record.holidays_in_range);
    if ("paid_mandatory_holidays" in record) inner += fields("Feriados pagados (no rebajan)", record.paid_mandatory_holidays);
    if ("rest_days_in_range" in record) inner += fields("Días descanso en rango", record.rest_days_in_range);
    if ("vacation_balance_before" in record) inner += fields("Saldo antes", record.vacation_balance_before);
    if ("vacation_balance_after" in record) inner += fields("Saldo después", record.vacation_balance_after);

    baseRows.push(section("Resumen (Vacaciones)", inner));
  }

  // INCAPACIDAD: mostrar emisor/cert + campos existentes (sin calcular)
...
async function loadLatestForSummary() {
  console.log("[LICENCIAS] loadLatestForSummary…");
  if (!st.employee?.uid) return;
  ...
  if (error) {
    console.error("[LICENCIAS] loadLatestForSummary error:", error);
    renderSummary(null);
    return;
  }
...
async function submitLeave() {
  ...
  try {
    ...
    const payload = {
      ...
      notes: notes || null,
      // status lo decide backend/DB si hay default; si existe, se guarda
    };
    ...
    // Leer la fila desde Supabase (por si triggers calcularon campos después)
    let finalRow = null;
    if (st.lastInsertedId) {
      finalRow = await fetchLeaveById(st.lastInsertedId);
    }
    if (!finalRow) finalRow = inserted;

    // Render resumen coherente (sin cálculos)
    ...
    // Ocultar emisor/cert por defecto
    toggleIssuerCertByUiType("");

    // Recargar historial (orden por created_at DESC)
    await loadHistory();
  } catch (err) {
...
async function loadHistory() {
  ...
  try {
    ...
    // Resumen principal SIEMPRE del último creado (primer fila por created_at desc)
...
async function bootLicencias() {
  ...
  // Cargar historial inicial (y también resumen correcto)
  await loadHistory();
}
